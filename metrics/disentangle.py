...
class Disentangle(object):
    ...
    def decompose(self, inputs, component):
        ...
        index = sorted(np.unique(self.bank[:, key]))  # component的类别,例如'iv'就是[0, 1, 2,..., 14]15种类别
        output = []
        for idx in index:
            same_class = [i for i, x in enumerate(self.bank[:, key]) if x == idx]  # 与idx相同类别的索引
    ...
    def extract(self, inputs, component="i"):
        ...
        if component == "ivt":
            ...
            component = [component] * len(inputs)  # 把这个字符串复制len(inputs)次, 如['iv'] -> ['iv', 'iv','iv',...]
